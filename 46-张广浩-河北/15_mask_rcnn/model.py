...
# 构建特征金字塔
# 返回p2_out, p3_out, p4_out, p5_out, p6_out不同尺寸特征图
class FPN(nn.Module):
...
# 根据正负样本和得分进行NMS保留正样本且得分高的ROI，同时使用预测得到的box偏移精炼锚框
...
# 二阶段检测 返回正负样本roi，正样本roi类别，正样本roi偏移，mask偏移  作为标签
def detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    """Subsamples proposals and generates target box refinment, class_ids,
    and masks for each.

    Inputs: 建议区域roi，gt类别id，gt的box，gt的mask
    ...
    Returns: Target ROIs and corresponding class IDs, bounding box shifts, 返回正样本roi信息对应类别，box偏移，mask偏移
    and masks.
    ...
    # Compute overlaps matrix [proposals, gt_boxes] 计算iou
    overlaps = bbox_overlaps(proposals, gt_boxes)

    # Determine postive and negative ROIs 通过iou判断正负样本
    ...
    if torch.nonzero(positive_roi_bool).size():
        ...
        # Assign positive ROIs to GT boxes.对齐正样本roi和gt 包括 类别，box
        positive_overlaps = overlaps[positive_indices.data,:]
        roi_gt_box_assignment = torch.max(positive_overlaps, dim=1)[1]
        roi_gt_boxes = gt_boxes[roi_gt_box_assignment.data,:]
        roi_gt_class_ids = gt_class_ids[roi_gt_box_assignment.data]

        # Compute bbox refinement for positive ROIs 计算正样本和gt之间的偏移
        ...
        # Assign positive ROIs to GT masks 对齐roi和gt的mask
        roi_masks = gt_masks[roi_gt_box_assignment.data,:,:]

        # Compute mask targets 计算mask的偏移
        boxes = positive_rois
    ...
    if positive_count > 0 and negative_count > 0:
        ...
        if config.GPU_COUNT:
            ...
            deltas = deltas.cuda()
            masks = masks.cuda()
    # 返回正负样本roi，正样本roi类别，正样本roi偏移，mask偏移
...
############################################################
# RPN网络，特征图作为输入，得到样本正负性，归一化后得分，box
class RPN(nn.Module):
    ...
    def forward(self, x):
        ...
        # Reshape to [batch, 2, anchors]展平得到所有锚框的2个分类结果
        rpn_class_logits = rpn_class_logits.permute(0,2,3,1)
        rpn_class_logits = rpn_class_logits.contiguous()
        rpn_class_logits = rpn_class_logits.view(x.size()[0], -1, 2)
        # 使用softmax进行二分类，得到每个锚框的正负样本性质
        ...
        # Reshape to [batch, 4, anchors]展平得到所有锚框的4个偏移结果
...
class Mask(nn.Module):
    ...
    # 多层卷积不改变特征图尺寸和维度，后续利用反卷积回到原始图像尺寸，利用1x1卷积通道数归为类别数。
    def forward(self, x, rois):
        x = pyramid_roi_align([rois] + x, self.pool_size, self.image_shape)
...
class MaskRCNN(nn.Module):
    ...
    # 检测 输入图像完成12阶段检测，进行还原得到roi，类别，置信度，mask
    def detect(self, images):
        """Runs the detection pipeline.

        images: List of images, potentially of different sizes.
        # 返回roi坐标，类别id，得分，mask
        ...
        # Process detections 处理检测结果得到  roi，类别，置信度，mask
        results = []
        ...
        return results
    # 输入图像进行目标检测（FPN->RPN->Propose->   一阶段   ->  检测头分类回归 + 检测头mask  二阶段）
    def predict(self, input, mode):
        ...
        # Feature extraction 特征金字塔进行特征提取，得到不同层次特征图
        ...
        # Loop through pyramid layers 对每层金字塔结果进行RPN产生所有anchor信息，包括（正负二分类得分，softmax后得分，box）
        ...
        #聚合所有尺度的anchor结果 正负二分类得分，softmax后得分，box
        ...
        # 得到ROI 对所有anchor进行nms保留下来的框为ROI（正样本+得分高）并且对于anchor使用预测得到的box偏移进行优化
        ...
        # 推断
        if mode == 'inference':
            # Network Heads二阶段使用特征图和roi得到各个roi分类得分，分类概率，回归结果
        ...
        # 训练 这里已经获得的roi一阶段结束
            ...
            # 二阶段检测 得到正样本roi类别，正样本roi偏移，mask偏移作为标签
            ...
            if not rois.size():
                ...
                # Network Heads 二阶段使用特征图和roi得到各个roi分类得分，分类概率，回归结果
                # Proposal classifier and BBox regressor heads 检测头得到类别，box，mask
                mrcnn_class_logits, mrcnn_class, mrcnn_bbox = self.classifier(mrcnn_feature_maps, rois)

                # Create masks for detections 二阶段使用特征图和roi得到各个roi的mask预测结果
                mrcnn_mask = self.mask(mrcnn_feature_maps, rois)
            # 返回 roi正负样本得分，roibox偏移，gt类别，预测分类得分，预测box偏移，预测mask偏移，
    ...
    def train_epoch(self, datagenerator, optimizer, steps):
        ...
        for inputs in datagenerator:
            ...
            # Run object detection 使用predict得到模型一二阶段预测结果
